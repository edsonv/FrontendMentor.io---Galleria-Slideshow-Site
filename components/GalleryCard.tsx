...
export const GalleryCard = ({
  ...
  return (
    <Link
      href={`/${originalIndex}`}
      key={item.name}
      className="card relative hover:mask-[linear-gradient(to_top,rgba(0,0,0,0.9)_16%,rgba(0,0,0,.5)_100%)]"
    >
      ...
      <div className="absolute bottom-0 left-0 z-20 p-4">
        ...
        <div className="text-preset-5 mt-1 text-white">{item.artist.name}</div>
      </div>
